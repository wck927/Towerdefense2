...
Tower::Tower(QPoint pos, MainWindow *game, const QPixmap &sprite/* = QPixmap(":/image/TT.png"*/)
    : m_attacking(false)
    , m_Range(100)
    , m_damage(15)
    , m_Rate(700)
...
void Tower::checkEnemyInRange()
{
    if (m_chooseEnemy)
    {
        ...
        // 如果敌人脱离攻击范围
        if (!collisionWithCircle(m_pos, m_Range, m_chooseEnemy->pos(), 1))
            losingEnemy();
    }
    else
    {
        ...
        {
            if (collisionWithCircle(m_pos, m_Range, enemy->pos(), 1))
            {
